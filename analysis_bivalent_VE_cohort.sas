/*********************************************************************************************************************************************************
Program: analysis_bivalent_VE_cohort.sas

Purpose: 
    Moderna bivalent cohort analysis:
	For VE cohort: Bivalent-vaccinated vs. unvaccinated
...
%let infile = in.covariates_cohort_VE_CR;
...
* Run baseline table macro;
	%baselinetable_unvac(data_baseline, table2, ARM_n);
	title 'Table 2';
...
* Check covariates with ASD>0.1 for model adjustment;
	proc sql;
		...
		select a.VarName, a.Stddiff_4dec as ABS_4dec, b.ABS as ABS_table2  
		from table2_std a
		left join table2 b on a.VarName=b.var
		where (a.Stddiff_4dec>0.1 or .<a.Stddiff_4dec<-0.1) and b.ABS^='N/A'
		order by VarName;
	quit;
	*age_grp Race_eth BMI_grp Charlson_wt Diabetes Frailty IC Kidney Lung Medicaid VA_grp h_med_income hist_covid_diag hist_covid_test medical_center_and_area preventive_care smoking;
	*Look out for continuous vs categorical vars and vars that are only applicable to subgroups: Frailty_index age days_since_hist_covid_c;
...
* Prepare data for analysis;
	...
	* Collapse vars for convergence issues;
		...
		race_eth_comb3 = race_eth; if race_eth in ('Non-Hispanic Asian','Non-Hispanic Black','Other/Unknown') then race_eth_comb3='Other/Unknown';
		if race_eth='Hispanic' then hispanic='Yes'; else hispanic='No';
		...
		VA_grp_comb2 = VA_grp; if VA_grp in ('0','1-4','5-10') then VA_grp_comb2='0-10';
		BMI_grp_comb5 = BMI_grp; if BMI>=30 then BMI_grp_comb5='>=30';
		BMI_grp_comb4 = BMI_grp_comb5; if .<BMI<25 then BMI_grp_comb4='<25';
		if frailty in (3,4) then frailty_comb2='Q3/Q4'; else frailty_comb2='Q1/Q2';
		charlson_wt_comb2 = charlson_wt; if charlson_wt in ('0','1') then charlson_wt_comb2='0-1';
    run;


***************************************************************;
** Table 5. VE against COVID-19 hospitalization overall and by subgroups;

* Hospitalization rVE overall (Collapsed age_grp and dropped h_med_income and med center for all hosp models); 
	%let varlist=age_grp_comb4 sex race_eth index_month BMI_grp smoking Charlson_wt Frailty Kidney Lung Diabetes IC hist_covid_diag hist_covid_test VA_grp preventive_care Medicaid antiviral;
    %cohort_analysis(data_analysis, covid_hosp, &exp., &unexp., py_hosp, lpy_hosp, py_hosp_mons, hosp_VE, survivalplot='Y'); *change survivalplot=y;
* Hospitalization rVE by subgroups;
	* Age (No cases in 6-17yo, will get warning. Adjusted for continuous age. Collapsed race_eth and VA_grp. For 18-44, dropped IC, Kidney, Lung, smoking, and further collapsed race_eth, BMI_grp, Frailty);
	data data_analysis_6to44 data_analysis_45to74 data_analysis_75up; 
	set data_analysis; 
	if .<age<45 then output data_analysis_6to44;
	if 45<=age<75 then output data_analysis_45to74;
	if age>=75 then output data_analysis_75up;
	run;
	%let varlist=sex hispanic index_month BMI_grp_comb4 Charlson_wt frailty_comb2 Diabetes hist_covid_diag hist_covid_test VA_grp_comb3 preventive_care Medicaid antiviral;
	%let age_term=age;
    %cohort_analysis(data_analysis_6to44,  covid_hosp, &exp., &unexp., py_hosp, lpy_hosp, py_hosp_mons, age_grp_6to44, subgroup='Y', by=age_grp);
	%let varlist=sex race_eth_comb4 index_month BMI_grp smoking Charlson_wt Frailty Kidney Lung Diabetes IC hist_covid_diag hist_covid_test VA_grp_comb3 preventive_care Medicaid antiviral;
    %cohort_analysis(data_analysis_45to74, covid_hosp, &exp., &unexp., py_hosp, lpy_hosp, py_hosp_mons, age_grp_45to74, subgroup='Y', by=age_grp); 
	...
	* Race/ethnicity (Collapsed BMI_grp, frailty, age_grp. Other/unknown has convergence issues, but not included in results);
	%let varlist=age_grp_comb3 sex race_eth index_month BMI_grp_comb5 smoking Charlson_wt Frailty_comb2 Kidney Lung Diabetes IC hist_covid_diag hist_covid_test VA_grp preventive_care Medicaid antiviral;
    %cohort_analysis(data_analysis, covid_hosp, &exp., &unexp., py_hosp, lpy_hosp, py_hosp_mons, race_eth, subgroup='Y', by=race_eth); 
	* IC status (Adjusted for IC subconditions in 'Yes' group. Collapsed race_eth, VA_grp. Dropped smoking for 'Yes' group);
	data data_analysis_IC0; set data_analysis; where IC=0; run;
	%let varlist=age_grp_comb4 sex race_eth_comb4 index_month BMI_grp smoking Charlson_wt Frailty Kidney Lung Diabetes IC hist_covid_diag hist_covid_test VA_grp_comb3 preventive_care Medicaid antiviral;
	...
	%let varlist=age_grp_comb4 sex race_eth_comb4 index_month BMI_grp Charlson_wt Frailty Kidney Lung Diabetes IC hist_covid_diag hist_covid_test VA_grp_comb3 preventive_care Medicaid antiviral IC_DX IC_HIV IC_MEDS IC_TX;
    %cohort_analysis(data_analysis_IC1, covid_hosp, &exp., &unexp., py_hosp, lpy_hosp, py_hosp_mons, IC1);
	* History of COVID (Adjusted for days_since_hist_covid_c and collapsed race_eth, BMI_grp, fraility in 'Yes' group);
	...
	%let varlist=age_grp_comb4 sex race_eth_comb4 index_month BMI_grp_comb4 smoking Charlson_wt frailty_comb2 Kidney Lung Diabetes IC hist_covid_diag hist_covid_test VA_grp preventive_care Medicaid antiviral days_since_hist_covid_c;
...
* Hospitalization by time since bivalent dose;
    ...
    * Calculate log person year for incidence rate calculation in macro;
	...
	%let varlist=age_grp_comb4 sex race_eth index_month BMI_grp smoking Charlson_wt Frailty Kidney Lung Diabetes IC hist_covid_diag hist_covid_test VA_grp preventive_care Medicaid antiviral;
    %cohort_analysis(data_analysis_bytime, covid_hosp1, &exp., &unexp., py_hosp1, lpy_hosp1, NA, time_grp1, debug='Y');
    %cohort_analysis(data_analysis_bytime, covid_hosp2, &exp., &unexp., py_hosp2, lpy_hosp2, NA, time_grp2, debug='Y'); 
    %cohort_analysis(data_analysis_bytime, covid_hosp3, &exp., &unexp., py_hosp3, lpy_hosp3, NA, time_grp3, debug='Y'); 
	*Collapsed age_grp, race_eth, BMI_grp. Dropped smoking;
	%let varlist=age_grp_comb3 sex race_eth_comb4 index_month BMI_grp_comb4 Charlson_wt Frailty Kidney Lung Diabetes IC hist_covid_diag hist_covid_test VA_grp preventive_care Medicaid antiviral;
    %cohort_analysis(data_analysis_bytime, covid_hosp4, &exp., &unexp., py_hosp4, lpy_hosp4, NA, time_grp4, debug='Y'); 

* Set results together;
    data blank; by=' '; run;
    data table5; 
	length by $40 description $40;
    set rslt_hosp_VE blank
		rslt_age_grp_6to44 rslt_age_grp_45to74 rslt_age_grp_75up blank 
		rslt_sex blank
		rslt_race_eth blank
		rslt_IC1 rslt_IC0 blank
		rslt_hist1 rslt_hist0 blank
		rslt_time_grp1 rslt_time_grp2 rslt_time_grp3 rslt_time_grp4
		;
    order=_N_;
    ...
    run;
    proc sort data=table5; by order; run;
...
***************************************************************;
** Table 6. VE against medically attended SARS-CoV-2 infection and COVID-19 hospital death;

* Medically attended infection (Not adjusted for antiviral);
	%let varlist=age_grp sex race_eth index_month BMI_grp smoking Charlson_wt Frailty Kidney Lung Diabetes IC hist_covid_diag hist_covid_test VA_grp preventive_care Medicaid h_med_income medical_center_and_area;
    %cohort_analysis(data_analysis, covid_infec_all, &exp., &unexp., py_infec_all, lpy_infec_all, py_infec_all_mons, infec_all_VE, survivalplot='Y');
    %cohort_analysis(data_analysis, covid_infec_edu, &exp., &unexp., py_infec_edu, lpy_infec_edu, py_infec_edu_mons, infec_edu_VE, survivalplot='Y');
* Hosp death (Collapsed age_grp, race_eth, BMI_grp. Dropped med center and h_med_income);
	%let varlist=age_grp_comb3 sex race_eth_comb4 index_month BMI_grp_comb5 smoking Charlson_wt Frailty Kidney Lung Diabetes IC hist_covid_diag hist_covid_test VA_grp preventive_care Medicaid antiviral;
	%cohort_analysis(data_analysis, covid_hosp_death, &exp., &unexp., py_dead, lpy_dead, py_dead_mons, dead_VE, survivalplot='Y');

* Set results together;
    data blank; by=' '; run;
    data table6; 
    set Rslt_infec_all_VE Rslt_infec_edu_VE
		Rslt_dead_VE;
    drop adjVE_adjCI separator4; *dropped adjVE_adjCI for this study as no multi-testing adjustment is planned;
    run;
